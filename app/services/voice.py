...
def create_subtitle(sub_maker: submaker.SubMaker, text: str, subtitle_file: str):
    """
    直接使用词语级别的字幕生成SRT文件
    不再按标点符号分割，而是直接使用sub_maker中的每个词
    """
    ...
    try:
        ...
        # 直接使用每个词作为一个字幕条目
        for idx, (offset, sub) in enumerate(zip(sub_maker.offset, sub_maker.subs), 1):
            start_time, end_time = offset
            
            # 直接使用100纳秒单位，mktimestamp函数期望的就是这个格式
            # 不需要转换为秒
            
            # 清理文本（移除可能的转义字符）
            sub_text = unescape(sub).strip()
            
            if sub_text:  # 只添加非空的字幕
                line = formatter(
                    idx=idx,
                    start_time=start_time,  # 直接传入100纳秒单位
                    end_time=end_time,      # 直接传入100纳秒单位
                    sub_text=sub_text,
                )
        ...
        if sub_items:
            ...
            try:
                ...
                logger.info(
                    f"completed, subtitle file created: {subtitle_file}, "
                    f"duration: {duration:.2f}s, total items: {len(sub_items)}"
                )
            except Exception as e:
                logger.error(f"failed to validate subtitle file: {str(e)}")
                # 不删除文件，保留以便调试
        else:
            logger.warning("No subtitle items generated")
...
def elevenlabs_tts(
    ...
    Args:
        ...
        voice_id: 语音ID
        voice_file: 输出的音频文件路径
        model_id: 模型ID，默认使用 "eleven_multilingual_v2"
        voice_stability: 语音稳定性 (0.0-1.0)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if characters and start_times and end_times:
                    ...
                    while i < len(characters):
                        ...
                        if char == " ":  # 遇到空格
                            if current_word:
                                # 向后查看是否有标点符号
                                j = i + 1
                                while j < len(characters) and j < len(end_times) and characters[j] in ".,!?;:'\"":
                                    current_word += characters[j]
                                    j += 1
                                
                                # 结束当前词语
                                if j-1 < len(end_times):
                                    word_end_time = end_times[j-1]
                                elif i < len(end_times):
                                    word_end_time = end_times[i]
                                else:
                                    word_end_time = end_times[-1] if end_times else 1.0
                                
                                # 将秒转换为100纳秒单位（与edge_tts兼容）
                                start_100ns = int(word_start_time * 10000000)
                                end_100ns = int(word_end_time * 10000000)
                                
                                sub_maker.subs.append(current_word)
                            ...
                            # 累积字符成词（包括标点符号）
                            if not current_word and i < len(start_times):
                    ...
                    logger.info(f"Created {len(sub_maker.subs)} word-level subtitles from character timestamps")
                else:
                    # 如果没有时间戳数据，回退到简单处理
                    logger.warning("No alignment data found, using fallback subtitle generation")
                    sub_maker.subs = [text]
                    # 假设10秒
